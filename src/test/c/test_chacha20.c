...
typedef struct {
    size_t bytes;
    char key[o1c_chacha20_KEY_BYTES * 2 + 1];
    char nonce[o1c_chacha20_NONCE_BYTES * 2 + 1];
    char *keystream;
    char *ciphertext;
} o1c_test_vector;

void run_checks(const o1c_test_vector *test) {
    uint8_t key[o1c_chacha20_KEY_BYTES], nonce[o1c_chacha20_NONCE_BYTES];
    uint8_t keystream[test->bytes], ciphertext[test->bytes];
    o1c_hex2bin(key, o1c_chacha20_KEY_BYTES, test->key, o1c_chacha20_KEY_BYTES * 2);
    o1c_hex2bin(nonce, o1c_chacha20_NONCE_BYTES, test->nonce, o1c_chacha20_NONCE_BYTES * 2);
