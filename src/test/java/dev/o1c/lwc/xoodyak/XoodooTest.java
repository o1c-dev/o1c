...
class XoodooTest {
    ...
    List<DynamicNode> loadTests() {
        List<DynamicNode> tests = new ArrayList<>(1024);
        InputStream stream = getClass().getResourceAsStream("xoodoo.txt.gz");
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new GZIPInputStream(stream), StandardCharsets.UTF_8))) {
            ...
            Pattern startLine = Pattern.compile("Start = ([0-9A-F]{96})");
            Pattern endLine = Pattern.compile("End = ([0-9A-F]{96})");
            ...
            for (int i = 0; i < 1024; i++) {
                ...
                assertTrue(matcher.matches());
                byte[] start = ByteOps.fromHex(matcher.group(1));
                matcher = endLine.matcher(reader.readLine());
                assertTrue(matcher.matches());
                byte[] end = ByteOps.fromHex(matcher.group(1));
                byte[] actual = new byte[end.length];
                // blank line
                reader.readLine();
